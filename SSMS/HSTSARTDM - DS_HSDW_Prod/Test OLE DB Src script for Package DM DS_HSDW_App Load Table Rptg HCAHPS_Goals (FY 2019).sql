...
--	,[@unit_department_translation].Epic_Department_Id
--	,[@unit_department_translation].Epic_Department_Name
--    ,Goal.[UNIT]
--  --  ,CASE
--	 --  WHEN Goal.UNIT IS NOT NULL AND Goal.UNIT IN ('All Units','All Service Lines') THEN Goal.UNIT
--	 --  ELSE TRIM([@unit_department_translation].Epic_Department_Name) + ' [' + TRIM([@unit_department_translation].Epic_Department_Id) + ']'
--	 --END AS Unit_Goals
--    --,COALESCE([@unit_translation].Unit, Goal.UNIT) AS Unit_Goals
--    ,COALESCE([@unit_response_unit_translation].Response_Unit, Goal.UNIT) AS Unit_Goals
--    ,Goal.[DOMAIN]
...
--LEFT OUTER JOIN @unit_response_unit_translation
--    ON [@unit_response_unit_translation].[Goal_Yr] = [Goal].[GOAL_YR]
--	AND [@unit_response_unit_translation].Goals_Unit = Goal.UNIT
--LEFT OUTER JOIN @unit_department_translation
--    ON [@unit_department_translation].[Goal_Yr] = [Goal].[GOAL_YR]
--	AND [@unit_department_translation].Goals_Unit = Goal.UNIT
...
--AND Goal.DOMAIN <> 'Pain Management';
...
--  --, CLINIC VARCHAR(250)
--  , INDEX IX_HCAHPS_PX_Goal_Setting_epic_id_FY19 NONCLUSTERED(GOAL_YR, Unit_Goals, Domain_Goals)
--);
...
(
  ...
  --, CLINIC VARCHAR(250)
  --, INDEX IX_HCAHPS_PX_Goal_Setting_epic_id_FY19 NONCLUSTERED(GOAL_YR, Unit_Goals, Domain_Goals)
  , INDEX IX_HCAHPS_PX_Goal_Setting_epic_id_FY19 NONCLUSTERED(GOAL_YR, UNIT, Domain_Goals)
);
INSERT INTO @HCAHPS_PX_Goal_Setting_epic_id
SELECT
     goals.GOAL_YR
    ,goals.SERVICE_LINE
	,goals.ServiceLine_Goals
	,CAST(NULL AS VARCHAR(255)) AS Epic_Department_Id
...
--AND ((goals.Unit_Goals IS NOT NULL) AND (goals.Unit_Goals NOT IN ('All Units','All Service Lines')))
AND ((goals.UNIT IS NOT NULL) AND (goals.UNIT NOT IN ('All Units','All Service Lines')))
...
SELECT DISTINCT
  ...
  --, CAST(NULL AS VARCHAR(8)) AS Epic_Department_Id
  --, CAST(NULL AS VARCHAR(30)) AS Epic_Department_Name
  , goals.Epic_Department_Id
  , goals.Epic_Department_Name
  , CAST(goals.Domain_Goals AS VARCHAR(150)) AS DOMAIN
  , CAST(goals.GOAL AS DECIMAL(4,3)) AS GOAL
  , CAST(GETDATE() AS SMALLDATETIME) AS Load_Dtm
FROM @HCAHPS_PX_Goal_Setting_epic_id goals
--WHERE goals.Unit_Goals = 'All Units' AND goals.GOAL_YR = 2019
...
--WHERE goals.Unit_Goals = 'All Service Lines' AND GOAL_YR = 2019
WHERE goals.UNIT = 'All Service Lines' AND GOAL_YR = 2019
AND goals.DOMAIN IS NOT NULL
) all_goals;
